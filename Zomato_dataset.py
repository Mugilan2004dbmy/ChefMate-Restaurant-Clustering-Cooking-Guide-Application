...
@st.cache_data
def load_data():
    df = pd.read_csv(r"C:\Users\MUGILAN\Downloads\Zomato_cluster_data.csv")
    return df

@st.cache_resource
def load_model():
    with open(r"C:\Users\MUGILAN\Downloads\kmeans_model.pkl", 'rb') as file:
        kmeans = pickle.load(file)
    with open(r"C:\Users\MUGILAN\Downloads\onehot_encoder.pkl", 'rb') as file:
...
st.title("🍽 Foodie Favorites Recommendation App 🍽")
        ...
        if not recommendations.empty and all(col in recommendations.columns for col in ['Restaurant_name', 'Cuisines', 'Aggregate_rating', 'City', 'Average_Cost_for_two', 'Currency']):
            for index, row in recommendations.iterrows():
                ...
                <div class="recommendation">
                    ...
                    <p><span class="cuisine"><strong>Cuisines:</strong> {row['Cuisines']} 🌮</span></p>
